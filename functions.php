...
function minimum_site_tagline() {
	...
	genesis_structural_wrap( 'site-tagline' );
		...
		printf( '<p><span class="yellow">CONTACT US TODAY!</span></br>
Get your <strong>FREE</strong> Estimate</p>', genesis_attr( 'site-description' ), esc_html( get_bloginfo( 'description' ) ) );
		echo '</div>';
	
		printf( '<div class="site-tagline-right free-est-1" %s>', genesis_attr( 'site-tagline-right' ) );
...
genesis_register_sidebar( array(
	'id'          => 'home-bkgrd-text',
	'name'        => __( 'Home Background Text', 'minimum' ),
	'description' => __( 'Widgets placed here will appear on top of home background image.', 'minimum' ),
) );

genesis_register_sidebar( array(
	'id'          => 'phone-number-bottom',
	'name'        => __( 'Bottom Phone Number', 'minimum' ),
	'description' => __( 'Widgets placed here will appear in the phone number region.', 'minimum' ),
) );
...
function minimum_home_bkgrd_image_overlay() {
	if (is_home() || is_front_page()) {
		...
		genesis_structural_wrap( 'home-bkgrd-image-overlay' );
 
			printf( '<div %s>', genesis_attr( 'home-bkgrd-image-overlay' ) );
				genesis_widget_area( 'home-bkgrd-image' );
				genesis_widget_area( 'home-bkgrd-text' );
			echo '</div>';
 
		genesis_structural_wrap( 'home-bkgrd-image-overlay', 'close' );
		echo '</div>';
	}
}

function minimum_after_content_number(){

 
			printf( '<div id="bottom_phone">' );
				genesis_widget_area( 'phone-number-bottom' );
			echo '</div>';


}
add_action( 'genesis_before_footer', 'minimum_after_content_number', 9);